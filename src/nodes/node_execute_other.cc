...
class FuncNode : public Node {
 ...
 public:
  ...
  {
    loop_body_tree_name_ = std::move(decl.add_tree_selector("Тело функции", std::move(loop_body_tree_name_)));
    const TreePtr tree = find_tree(loop_body_tree_name_);
    
    decl.add_input<int>("Исполнить");
  ...
  void execute(ExecutionContext &context) const override
  {
    ...
    const bool do_execute = context.get_input<int>("Исполнить");
...
void register_node_execute_other_node_type()
{
  register_node_type("Функция", []() -> NodePtr {
    return std::make_unique<node::execute_other::FuncNode>();
  });
}
